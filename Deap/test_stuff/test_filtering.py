...
def open_bag(path, plot = False, thr_bucket = True):
	...
	def new_filter(nav_data):
		...
		cut_test = [0.01, 0.05, 0.1, 0.2, 0.4]

		fs = 1/steps #freq
		Q = 20 		 #qu
		for cutoff in cut_test:
			#b, a = signal.iirnotch(cutoff, Q, fs=fs)
			b, a = signal.butter(order, cutoff, btype='low', analog=False, output='ba')
	...
	if plot:
		...
		#plt.plot([nav_data[0,0],nav_data[0,-1]],[nav_data[1,0], nav_data[1,-1]])
