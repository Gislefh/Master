...
plt.figure()
plt.plot(Froude_force)
plt.plot(X_force[0])
plt.plot(np.ones(len(Froude_force))*0.4)
plt.plot(np.ones(len(Froude_force)))
plt.grid()
plt.show()
exit()
...
def stats(model, y, num_vars):
    # ---RMSE
    mse = np.sum((y-model)**2)/len(model)
    rmse = np.sqrt(mse)

   
    # r^2
    r_sq = r2_score(y, model)
    ad_r_sq = 1-((1-r_sq)*(len(model)- 1))/(len(model)- num_vars - 1)


    rss = np.sum((y-model)**2)
    sigma_sq = rss /(len(y)- num_vars- 1)

    aic = (1/(len(y)*sigma_sq)) *(rss + 2*num_vars*sigma_sq) 

    bic = (1/(len(y)*sigma_sq)) *(rss + np.log(len(y))*num_vars*sigma_sq)

    Cp = (1/len(y)) *(rss + 2*num_vars*sigma_sq)

    

    out = [rmse, Cp, ad_r_sq, r_sq, aic, bic]


    return out

du_force_stats = stats(du_force_array, X_force[3], 6)
dv_force_stats = stats(dv_force_array, X_force[4], 6)
dr_force_stats = stats(dr_force_array, X_force[5], 6)

du_ac_dyn_stats = stats(du_ac_dyn_array, X[3], 5)
dv_ac_dyn_stats = stats(dv_ac_dyn_array, X[4], 5)
dr_ac_dyn_stats = stats(dr_ac_dyn_array, X[5], 5)


du_st_model_stats = stats(eta_dot_nu_dot[3], X[3], 6) 
dv_st_model_stats = stats(eta_dot_nu_dot[4], X[4], 6) 
dr_st_model_stats = stats(eta_dot_nu_dot[5], X[5], 6) 

print(dr_force_stats)
print(dr_ac_dyn_stats)
#print(du_st_model_stats)
exit()
